...
int user_counter = 0;
pthread_mutex_t scan_lock, prnt_lock, con_lock;
////////////////////////////////////////////////////////
void *user(void *arg)
{
    string cmd;
    ...
    getline(cin, cmd);
    cmd.append(" ");
    cmd.append(to_string(user_counter));
    user_counter++;
    pthread_mutex_unlock(&scan_lock);
    //
    int time_delay = atoi((cmd.substr(0, cmd.find(" "))).c_str());
    sleep(time_delay);
    //
...
int main(int argc, char *argv[])
{
    ...
    getchar();
    /////
    pthread_t thread_ids_arr[M];

    for (int i = 0; i < M; i++)
    {
        pthread_t curr_tid;
        pthread_create(&curr_tid, NULL, user, NULL);
    ...
    int M;
    scanf("%d", &M);
    send_string_on_socket(socket_fd, to_string(M));
    cout << read_string_from_socket(socket_fd, buff_sz).first << endl;
    string cmds[M];
    getchar();
    for(int i =0; i< M;i++){
        getline(cin, cmds[i]);
        cmds[i].append(" ");
        cmds[i].append(to_string(i));
    }
    for(int i =0; i< M;i++){
        cout << cmds[i] << endl;
        send_string_on_socket(socket_fd, cmds[i]);
        int num_bytes_read;
        string output_msg;
